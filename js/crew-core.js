...
const CFG = CREW.CFG = {
  ...
  ladder:{
    standTread : 3,      // which tread he works from (0-based) — one down from the top
...
CREW.setThemeMix = function(t){
  themeMix.t = t;
  for(const k of ["hemi","key","fill","rim","amb"]) L[k].intensity = lerp(THEME_LIGHT[k], THEME_DARK[k], t);
...
CREW.layout = function(){
  ...
  S.treadH     = S.stature*0.435;          // = STEP_TREADS[CFG.ladder.standTread]
