...
/* =====================================================================
   HOP-UP  (v2)
   v1 used a five-tread A-frame he stood part way up, and that is geometrically hostile: every
   tread above his feet — and the top platform especially — occupies exactly the volume his
   shins are in, so the frame saws through the character no matter how the pose is tuned. No
   amount of offsetting fixes it, because the collision is with the design, not the numbers.

   v2 is a short three-step hop-up he stands on TOP of. Nothing exists above the soles of his
   feet, so the intersection is gone by construction. It is also what a painter actually
   carries one-handed for fascia height, and the shorter lift keeps the logo row from being
   driven up into the ceiling.

   Origin at the floor, centred between the front stiles. The tread side faces -Z, i.e. the
   wall, so he climbs it facing his work.
   ===================================================================== */
const STEP_TREADS = [0.100, 0.200, 0.300];
const STEP_TOP = 0.400;
CREW.makeLadder = function(){
  const g=new T3.Group();
  const sw=0.021;                       // stile thickness
  const wBot=0.150, wTop=0.124;         // half-widths, front frame tapers upward
  const zF=-0.020, zBackBot=0.205;      // where the back legs land
  ...
  /* the deck he stands on — a real platform, not a shelf */
  const deckD = 0.150, deckZ = zF + 0.052;
  const deck=box(M.wood, wTop*2+sw*1.6, 0.020, deckD);
  at(deck, 0, STEP_TOP, deckZ);
  g.add(deck);
  const nose=box(M.steelDk, wTop*2+sw*1.6, 0.007, 0.018);
  at(nose, 0, STEP_TOP+0.013, deckZ - deckD*0.5 + 0.009);
  g.add(nose);
  /* a low guard rail across the BACK of the deck. It reads as a hop-up rather than a stool,
     and because it lives behind him it can never cross his legs. */
  const railZ = deckZ + deckD*0.60, railH = 0.140;
  for(const sx of [-1,1]){
    const post=box(M.steel, 0.012, railH, 0.012);
    at(post, sx*(wTop*1.18), STEP_TOP+railH*0.5, railZ);
    g.add(post);
  }
  const rail=cyl(M.steel, 0.012, 0.012, wTop*2.36+0.024, 10);
  rail.rotation.z=Math.PI/2;
  at(rail, 0, STEP_TOP+railH, railZ);
  g.add(rail);
  ...
  /* spreader arms, so the A cannot close — kept low and outboard so they stay clear of him */
  for(const sx of [-1,1]){
    const arm=box(M.steel, 0.006, 0.006, 0.130);
    at(arm, sx*0.132, 0.150, zF+0.078);
    arm.rotation.x = 0.46;
    g.add(arm);
  }
  ...
  g.userData = {
    ...
    /* the sequence he actually climbs: floor, each tread, then the deck. Index nStep is the
       deck, which is where he works from. */
    nStep: STEP_TREADS.length,
    stepY: i => (i < 0 ? 0 : (i >= STEP_TREADS.length ? STEP_TOP : STEP_TREADS[i])),
    stepZ: i => (i >= STEP_TREADS.length ? deckZ : zF+0.026),
    treadY: i => STEP_TREADS[clamp(Math.round(i),0,STEP_TREADS.length-1)],
    treadZ: zF+0.026,
    deckZ,
    halfWAt: y => lerp(wBot, wTop, clamp(y/STEP_TOP,0,1)),
    /* carried by the back rail, at his side */
    grip: new T3.Vector3(0, STEP_TOP+0.140, deckZ + deckD*0.60),
