...
function buildSchedule(persona){
  ...
  for(let pass=0; pass<2; pass++){
    ...
    for(let i=0;i<S.perPass;i++){
      ...
      if(i === 0){
        if(!paint){
          /* The reveal beat. He walks to the board on the left and throws the mains, and the
             whole wall comes up out of black with every sign already on it. Panning a torch
             along the row was tried first and is worse: a long throw makes the cone enormous
             and grazing, so it lights nothing and hides everything behind it. */
          const mx = S.slots[0].x - S.slotW*0.95;
          move("walk", mx, null, { pass, slot:i });
          push("mains", 0.80, { pass, slot:i });
        }
...
const ST = {
  ...
  lampTarget:V(),
  busX:0,
  lastContact:null, onLadder:false, mains:0,
  seedTick:0,
};
...
/* index -1 is the floor, 0..n-1 the treads, n the deck he works from */
function stepW(i, side, out){
  const u = KIT.ladder.userData;
  const j = Math.min(i, u.nStep);
  const onDeck = j >= u.nStep;
  return ladderLocalW(-0.014 + (side||0)*(onDeck ? 0.058 : 0.046),
                      u.stepY(j), u.stepZ(j), out);
}
...
function climbFeet(p){
  /* p is how many steps up he is: -1 = both feet on the floor, nStep = both on the deck */
  const u = KIT.ladder.userData;
  let lowest = Infinity;
  for(const side of ["Left","Right"]){
    const lead = side === "Right" ? 0 : 0.5;
    const ph = Math.min(p + lead, u.nStep);
    const target = Math.min(Math.floor(ph), u.nStep);
    const frac = clamp((ph - target)*2, 0, 1);
    const from = target - 1;
    const a = stepW(from, side==="Left" ? 1 : -1, _a).clone();
    const b = stepW(target, side==="Left" ? 1 : -1, _b).clone();
...
/* the working stance: both feet flat on the deck, weight slightly apart. Nothing on the
   ladder is above this height, which is the whole reason the deck exists. */
function climbHold(){
  const u = KIT.ladder.userData;
  const fL = stepW(u.nStep, 1, _a).clone(), fR = stepW(u.nStep, -1, _b).clone();
  RIG.foot.Left.x  = fL.x; RIG.foot.Left.y  = fL.y; RIG.foot.Left.z  = fL.z;
  RIG.foot.Right.x = fR.x; RIG.foot.Right.y = fR.y; RIG.foot.Right.z = fR.z;
  for(const s of ["Left","Right"]){
    RIG.foot[s].lift = 0; RIG.foot[s].level = 0.92;
    RIG.foot[s].pole = _c.set(Math.sin(ST.ladderYaw), 0.22, Math.cos(ST.ladderYaw)).normalize().clone().negate();
  }
...
/* v2: the kettle never leaves his hand. v1 parked it on the ladder's top shelf, which is
   also where his shins were — and once the deck is the thing he stands on there is nowhere
   to park it anyway. Holding it is what a painter does, it gives the free arm a job, and it
   puts the paint within an easy dip of the brush. */
function heldKettle(dt){
  const shL = CREW.boneAt("LeftArm", _a);
  /* out on the camera side: held on his far side it disappears behind his own body and the
     dip reads as him poking a brush into nothing */
  const hand = _b.copy(shL)
    .addScaledVector(UP, -HUMAN.armLen*0.90)
    .addScaledVector(forwardVec(_c), S.stature*0.015)
    .addScaledVector(leftVec(_d), -S.stature*0.140);
  swingKettle(hand, dt || 1/60);
  handTo("Left", hand, _e.set(1,0,0), UP, POLE_DOWN, 1);
  return hand;
}
  ...
  apply(a, k, dt){
    ...
    switch(a.kind){
      ...
      case "climb": case "descend": {
        ...
        placeLadder(a.x1, 0);
        this.stowBrush();
        climbFeet(lerp(-1, KIT.ladder.userData.nStep, easeIO(kk)));
        RIG.lean = 0.22;
        /* the guard rail across the back of the deck is what a hand actually reaches for */
        const u = KIT.ladder.userData;
        handTo("Right", ladderLocalW(u.grip.x + 0.055, u.grip.y, u.grip.z, _a),
               _b.set(1,0,0).applyQuaternion(KIT.ladder.quaternion), UP, _c.set(0,-0.6,1), 1);
        heldKettle(dt);
        break;
      }
      /* loading the brush out of the kettle he is holding */
      case "dip": {
        holdOnLadder();
        heldKettle(dt);
        KIT.brush.visible = true; KIT.roller.visible = false;
        const dipP = kettleDipW(_a).clone();
        /* Dip on a DIAGONAL. Straight down would need the hand a whole brush-length above
           the paint — higher than his own shoulder, because the brush is long and the arm is
           short. Angling it puts the grip out to the side instead, which is how it is
           actually done and keeps the wrist below the shoulder. */
        const dipAim = _d.set(0.62, -0.78, 0.12).normalize();
        const rest = _b.copy(dipP).addScaledVector(dipAim, -S.stature*0.20);
        const tipT = k < 0.55 ? _c.copy(rest).lerp(dipP, easeIO(k/0.55))
                              : _c.copy(dipP).lerp(rest, easeIO((k-0.55)/0.45));
        const gripW = CREW.placeTool(KIT.brush, tipT, dipAim, _e.set(0,0,1));
      ...
      case "stroke": {
        holdOnLadder();
        const wash = a.layer === "wash";
        /* the kettle stays in the free hand while he brushes; the roller needs both, so it
           hangs at his side for that one */
        if(!wash) heldKettle(dt);
        workSurface(s, a, k, wash);
        break;
      }
...
const ELEC = {
  ...
  apply(a, k, dt){
    ...
    switch(a.kind){
      case "offstage": case "wait": case "walk": case "exit":
        ST.climb = 0; ST.kitOn = "hand";
        this.carryKit(dt, aheadTarget());
        break;
      /* throwing the mains */
      case "mains": {
        ST.climb = 0;
        const box = _e.set(CREW.wx(RIG.x - S.stature*0.30, CFG.layout.wallZ),
                           HUMAN.groundW + S.stature*0.60, CFG.layout.wallZ + 6).clone();
        aimLamp(box, dt);
        RIG.crouch = 0.10*Math.sin(Math.PI*k);
        /* right hand goes to the board and pushes the breaker up */
        const gripW = CREW.placeTool(KIT.plug, box, _a.set(-0.25,0.30,1).normalize(),
                                     _b.set(0,1,0));
        handTo("Right", gripW, CREW.toolShaft(KIT.plug,_c), UP, POLE_DOWN, 1);
        if(once(a, "__thrown", k > 0.34)){
          CREW.sfx.plug(); CREW.sfx.spark(); CREW.sfx.ignite();
          for(let i=0;i<18;i++){
            const ang = hash(i*3.7)*6.2832, sp = S.stature*(0.3+hash(i*5.1)*1.1);
            CREW.emit({ x:box.x, y:box.y, z:box.z,
                        vx:Math.cos(ang)*sp, vy:Math.abs(Math.sin(ang))*sp*0.8, vz:sp*0.4,
                        life:0.16+hash(i*9.1)*0.22, size:S.stature*0.011,
                        color:"#FFF6D2", kind:CREW.KIND.SPARK, grav:2100, drag:0.09 });
          }
        }
        /* the room comes up, with the stutter of a contactor pulling in */
        const lift = k < 0.34 ? 0
                   : k < 0.46 ? (hash(Math.floor(k*60)*7.7) > 0.45 ? 0.55 : 0.05)
                   : easeOut(clamp((k-0.46)/0.42, 0, 1));
        ST.mains = Math.max(ST.mains, lift);
        break;
      }
      case "shift": {
        ST.climb = 0;
...
const GROUND_ACTS = { walk:1, exit:1, offstage:1, wait:1, shift:1, setLadder:1, lift:1 };
...
/* v2: the lamp is held for the whole job too — same reason as the kettle, and it means the
   beam follows his body instead of sitting frozen on a shelf. `want` is a world point.
   snap skips the smoothing, for the pan across the row on entry. */
function aimLamp(want, dt, snap){
  const shL = CREW.boneAt("LeftArm", _a);
  const hand = _b.copy(shL)
    .addScaledVector(UP, -HUMAN.armLen*0.60)
    .addScaledVector(forwardVec(_c), S.stature*0.115)
    .addScaledVector(leftVec(_d), S.stature*0.055);
  if(snap) ST.lampTarget.copy(want);
  else ST.lampTarget.lerp(want, 1-Math.pow(0.0012, dt || 1/60));
...
/* The entrance pan. The signs are all physically on the wall from frame zero — they are just
   unlit, so nothing is visible until light falls on them. Walking the beam out along the row
   and back is what tells the audience that: each logo surfaces as the cone crosses it and
   drops away again behind it, which reads as "there is a wall of signs here in the dark",
   not as "logos are being created one at a time". */
/* while walking, the torch falls on the wall just ahead of him — a short throw, so the cone
   stays a torch beam instead of a room-filling wedge */
function aheadTarget(){
  const dir = ST.speed < -1 ? -1 : 1;
  return _e.set(CREW.wx(RIG.x + dir*S.stature*0.55, CFG.layout.wallZ),
                CREW.wy(S.rowY + S.slotH*0.30, CFG.layout.wallZ),
                CFG.layout.wallZ).clone();
}
function lampAtSlot(s, dt){
  if(!s) return aimLamp(ST.lampTarget, dt);
  return aimLamp(CREW.slotPointW(s, 0.5, 0.55, 0, _e).clone(), dt);
}
function stowPlug(){
...
function evaluate(dt){
  ...
  if(a.slot != null){
    ...
    else if(a.kind === "setLadder") x = lx + SIDE;
...
function resetWorld(){
  SHOW.t = 0;
  ST.climb = 0; ST.busX = -S.W*0.10; ST.kitOn = "hand"; ST.mains = 0;
...
function tick(dt){
  ...
  if(SHOW.t >= SHOW.dur){ SHOW.t -= SHOW.dur; resetSlots(); ST.busX = -S.W*0.10;
                          CREW.clearDecals();
                          for(const b of (KIT.branch||[])){ b.mesh.visible=false; b.on=false; } }
  /* the mains stay up for the rest of the loop once he has thrown them */
  CREW.setMainsLift(ST.mains);
...
const BEAT = { walk:"walking on", exit:"walking off", shift:"moving the ladder",
  ...
  dieout:"flickering out", spin:"turning the sign", mains:"throwing the mains" };
